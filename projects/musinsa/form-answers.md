...
Apple evidence 14280470370은 상품상세 품절 오표시, 결제 지연, 목록·상세 전환 지연, 발열의 4개 독립 티켓과 4개 증상별 회귀 테스트 초안으로 분리됐습니다. evidence 보존율은 1.0이지만 device·OS가 synthetic_test_context이므로 4건 모두 TEST_SCENARIO_ONLY이며 실제 환경 확인 전 READY_FOR_REPRO라고 주장하지 않습니다. 환경이 빈 Google Play 입력의 3개 티켓은 NEEDS_REPRO_AND_ENVIRONMENT_CONFIRMATION, invalid 입력은 exit code 2였습니다. 기존 14개와 신규 6개 등 총 20개 테스트에서 synthetic READY 금지, 원문 substring·offset·해시, 증상별 절차·oracle, normalized/raw 분리, evidence ID 보존과 root_cause 금지를 확인했습니다. 실제 앱 재현과 내부 우선순위는 검증하지 않았습니다. 메인 작업 로그는 최종 산출물 확정 cutoff까지 원본 byte-for-byte snapshot으로 보존하고 이후 패키징·업로드 확인 대화는 제출 범위에서 제외합니다.
